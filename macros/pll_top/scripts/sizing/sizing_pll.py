...
# Description: Analytical (gm/ID) sizing of the pll macro.
#
# The specifications live in specs_pll.py — edit that file, not this
...
# scripts/sizing/sizing_pll.md (specs + computed sizing).
#
...
import specs_pll as specs

SPECS_FILE = Path(__file__).resolve().parent / "specs_pll.py"
REPORT_FILE = Path(__file__).resolve().parent / "sizing_pll.md"
# ============================================


def main():
    r = Report("pll", generator=Path(__file__).name)
    r.specs(SPECS_FILE)

    # ============================================
    # Load SG13G2 data tables
    # ============================================
    lv_nmos = load_table("sg13cmos5l_lv_nmos")
    lv_pmos = load_table("sg13cmos5l_lv_pmos")
    # hv_nmos = load_table("sg13cmos5l_hv_nmos")
    # hv_pmos = load_table("sg13cmos5l_hv_pmos")
